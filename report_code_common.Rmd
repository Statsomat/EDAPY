...
df_num_stats["MAD"] = df_num.apply(lambda x: stats.median_abs_deviation(x, scale="normal"))    
...
df_cat_stats["N Missing"] = df_cat.apply(lambda x: (x == 'NA').sum() + x.isna().sum())
...
df_cat_miss.replace(to_replace='NA', value='Missing', inplace=True) 

## Restrict to first characters
df_cat_miss = df_cat_miss.astype("string")

...
if (exists("df_num")) main$df_num <- r_to_py(df_num)
...
if (exists("df_num")){
  if (dim(df_num)[2] != 0L){
    ...
    cat("Variables are sorted alphabetically. Missings are omitted in stats (besides N Unique). CV only for positive variables. ", fill=TRUE)
...
def qqplot(var, ax, xlabel, ylabel):
    pp = sm.ProbPlot(df_num.iloc[:,var])
    pp.qqplot(marker='.', ax = ax, line = 'q', 

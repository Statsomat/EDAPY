...
df_num_stats["N Unique"] = df_num.nunique(dropna=False)
...
if (exists("df_num")){
  if (dim(df_num)[2] > 1){
    ...
    if(dim(df_num)[2]> 1){
      cat("Details: Density Histograms. One large figure per page for each variable. Variables are sorted alphabetically. The blue line represents the normal density approximation. The green line represents a special kernel density approximation.  ", fill=TRUE)
      cat("\\newline", fill=TRUE)
...
if (exists("df_num")){
  if (dim(df_num)[2] == 1){
    ...
    if(dim(df_num)[2]==1){
      ...
      cat("\\newline", fill=TRUE)
      cat("Figure on next page. ", fill=TRUE)
    }
  } 
}
```
...
def ecdf(ax, var, ylabel, xsize):
    ...
    ax.set_xlabel(df_num.columns[var], fontsize=xsize)
    ax.set_ylabel(ylabel) 
    title = "ECDF Plot of " + str(df_num.columns[var])
    ax.set_title(title)
...
```{python, eval=eval_num, echo = FALSE, comment='', message = FALSE, error = TRUE, warning=FALSE, fig.dim = c(14,8)}
## QQ Large
def qqplot(var, ax, xlabel, ylabel):
    sm.qqplot(df_num.iloc[:,var], ax = ax, 
              line = "q", 
              marker='.', markerfacecolor='#2fa42d', 
...
for var in range(df_num.shape[1]):
  fig, ax = plt.subplots(nrows = 1, ncols=1)
  qqplot(var, ax, "Normal Quantiles", "Sample Quantiles for ")
  plt.show()
  if (var==0):
    plt.plot()
```
...
df_cat_stats.index = cnames[0:csize]
